...
/**
  Downsamples the input buffer and feeds it to listeners
*/
class RecorderProcessor extends AudioWorkletProcessor {
  ...
  constructor() {
    super()
    this.initBuffer()
    //Retrieved from worklet global context
    this._inputFrameRate = sampleRate
    this._resampler = new Resampler(this._inputFrameRate, 16000, 1, this._buffer)
  }
   ...
   */
  process(inputs) {
    // Grabbing the 1st channel (mono only)
  ...
  flush() {
    ...
    const resultLength = this._resampler.resampler(Math.min(buffer.length, this._bytesWritten))
    if (resultLength > 0) {
      this.port.postMessage(this._resampler.outputBuffer.slice(0, resultLength))
    }
    this.initBuffer()
  }
}
